...
\begin{document}
    ...
    \begin{ejercicio}{2}
        ...
        \begin{enumerate}
            ...
            %\item Para todo $n \in \mathbb{N}$, se cumple que: $\displaystyle \sum_{k=0}^{n} (2k+1) = (n+1)^2$.
    ...
    \begin{ejercicio}
        ?`Cuantos triángulos, equiláteros, orientados hacia arriba, distintos, que téngan sus véretices en la siguiente figura existen?.
        \begin{center}
            \begin{tikzpicture}[scale=0.5]
                ...
                \foreach \i in {0,...,15} {
                \foreach \j in {0,...,\i} {
                    % Coordenadas base para cada triángulo
                    \coordinate (A) at ({(\j - \i/2)*\l}, {-\i*sqrt(3)/2*\l});
                    \coordinate (B) at ({((\j+1) - \i/2)*\l}, {-\i*sqrt(3)/2*\l});
                    \coordinate (C) at ({(\j+0.5 - \i/2)*\l}, {(-\i*sqrt(3)/2 + sqrt(3)/2)*\l});
