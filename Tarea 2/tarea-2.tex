...
\begin{document}
    ...
    \begin{ejercicio}{1}
        Sean $A,B$ conjuntos y $f:A \to B$ una función. La relación $\sim \subseteq A \times A$ definida por $x \sim y \; \Leftrightarrow \; f(x)=f(y)$ es de equivalencia (\textit{¿por qué?}). Sea $\pi:A \to \quot{A}{\sim}$ definida por $f(x) = [x]$. Demuestre que $\pi$ es biyectiva si y sólo si $R=\id_A$.
    \end{ejercicio}
